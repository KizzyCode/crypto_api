...
pub trait StreamingCipher {
	...
	fn init(&mut self, key: &[u8], nonce: &[u8]) -> Result<(), Box<dyn Error>>;
	...
	fn update<'a>(&mut self, input: impl Iterator<Item = &'a u8>,
		output: impl Iterator<Item = &'a mut u8>) -> Result<usize, Box<dyn Error>>;
	
	/// Finishes the operation and writes the pending bytes to `output` and returns the amount of
	/// bytes written
	fn finish<'a>(&mut self, output: impl Iterator<Item = &'a mut u8>) -> Result<usize, Box<dyn Error>>;
}
...
pub trait StreamingAeadCipher: StreamingCipher {
	...
	fn update_ad<'a>(&mut self, ad_input: impl Iterator<Item = &'a u8>) -> Result<(), Box<dyn Error>>;
}